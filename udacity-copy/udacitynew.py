...
def l(url):
	...
	for link in e.find_elements_by_xpath("//*[@id='main-layout-sidebar']/div/div[1]/ol/li/a"):
		href=link.get_attribute('href').replace('https://classroom.udacity.com/','')
                link.click()
		time.sleep(0.3)
		save(href,e.page_source.replace('<track label="Chinese" ','<track label="Chinese" default ').replace('video class="video__el" crossorigin="anonymous">','video class="video__el" controls controlsList="nodownload" autoplay="autoplay" crossorigin="anonymous">') )
def o(url,i):
	lk=""
	e.get(url)
	time.sleep(3)
...
e=webdriver.Chrome()
e.set_window_size(1920, 550)
e.implicitly_wait(10)
login("qq21008037@iigogo.com","aa123456")
time.sleep(5)
for i in range(1110,1300):
	o("https://classroom.udacity.com/courses/ud"+str(i),str(i))
