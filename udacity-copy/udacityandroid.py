...
def l(url):
	...
	for index in range(40):
		...
		try:
			...
			f=open("c://android//"+str(index1)+"_"+str(index)+".htm",mode="w",encoding="UTF-8")
			f.write( page )
			f.close()
		except:
			break
...
f = open('c://1udacityandroid.txt','r')  
for line in open('c://1udacityandroid.txt'):
	linae=f.readline()
	if line>"":
		l(line)
